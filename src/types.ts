...
type DaemonToWorkerBase =
  ...
  * worker to reattach; PTY keeps its historical cold-resume behavior. */
   ...
   *  `model` / `reasoningEffort` / `fresh` 只由 daemon 直控命令使用：
   *  它们更新 worker 的 lastInitConfig 后再 respawn，避免把 `/model ...`
   *  `/effort ...` `/new` 作为模型 turn 发送。 */
  | { type: 'restart'; reason?: 'operator' | 'cli_crash'; attemptId?: string; updateWorkingDir?: string; env?: Record<string, string> | null; model?: string | null; reasoningEffort?: Session['reasoningEffort'] | null; fresh?: boolean }
