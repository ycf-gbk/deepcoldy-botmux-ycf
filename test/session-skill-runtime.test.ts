...
describe('session skill runtime preparation', () => {
  ...
  it('loads the shared runtime skills even when the bot has no per-bot policy', () => {
    ...
    expect(result.prompt).toContain('botmux skill show botmux-send');
    expect(result.manifest?.prioritySkills.map((s) => s.name)).toContain('botmux-send');
  });
  ...
  it('writes manifest and appends catalog for configured priority skills', () => {
    ...
    expect(readSessionSkillManifest('s2')?.prioritySkills.map((s) => s.name)).toContain('deploy');
  });
  ...
  it('refreshes a prompt-less CLI generation and removes stale session skills', () => {
    ...
    expect(readSessionSkillManifest('refresh-session')?.prioritySkills.map(skill => skill.name)).toContain('browser');
    ...
    expect(readSessionSkillManifest('refresh-session')?.prioritySkills.map(skill => skill.name)).toContain('botmux-send');
  });
});
